# Galeria de Fotos Interativa - Desafio Tecnico

Esta aplicacao web foi desenvolvida para visualizacao e filtragem de imagens em um layout responsivo. O projeto foca na utilizacao de tecnologias front-end essenciais para criar uma interface funcional e intuitiva.

- Tecnologias Utilizadas
- HTML5: Estruturacao semantica da pagina.
- CSS3: Estilizacao personalizada, incluindo gradientes e efeitos de interatividade.
- JavaScript (ES6+): Logica de busca, filtragem de dados e manipulacao do DOM.
- Bootstrap 5: Sistema de grid para responsividade e componentes de interface.
- UIcons: Biblioteca utilizada para o icone de pesquisa.

************************************************************************************************8
- Funcionalidades Implementadas
- Cabecalho: Titulo centralizado com estilizacao via gradiente.
- Barra de Busca: Campo de texto para pesquisa por palavras-chave com botao de acao.
- Filtro em Tempo Real: Atualizacao automatica da galeria conforme o usuario digita.
- Grid de Fotos: Exibicao de imagens com nomes centralizados e organizacao responsiva.
- Efeitos Visuais: Feedback de interacao ao passar o mouse (hover) sobre os elementos.
- Mensagem de Feedback: Notificacao exibida quando nenhum resultado corresponde a busca.
- Rodape: Informacoes de copyright fixadas na base da aplicacao.

************************************************************************************************8
- Como Executar o Projeto
- Faca o download ou clone este repositorio em sua maquina local.
- Localize o arquivo index.html na pasta raiz.
- Abra o arquivo em qualquer navegador web atualizado.

************************************************************************************************8
- Dados da Aplicacao
O projeto utiliza um conjunto de dados estaticos contendo titulos e URLs de imagens, garantindo que a pesquisa funcione de forma precisa conforme os requisitos do exercicio.

---
Desenvolvido por Ana Carolina - 2026
